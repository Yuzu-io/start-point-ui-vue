<template>
  <n-modal v-model:show="show" title="添加角色" preset="card" style="width: 500px" @ok="onSubmit">
    <n-form
      ref="formRef"
      :model="formState"
      :rules="rules"
    ...
    >
      ...
      <n-form-item label="权限" path="routesIdList">
        <n-tree
          ...
          default-expand-all
          :data="treeData"
          :default-selected-keys="formState.routesIdList"
...
const formRef = ref()
const formState = ref<AddRoleParams>({
  roleName: '',
  roleKey: '',
  status: '0',
  orderIndex: 1,
...
const onSubmit = () => {
  formRef.value
    .validate()
    .then(async () => {
      const result = await addRoleApi(formState.value)
      if (result.code === 200) {
